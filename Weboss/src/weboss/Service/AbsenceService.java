       ...
       public void AddAbsence(Absence t) {
        ...
        try {
            pst = cnx.prepareStatement(req);
             ...
             pst.setInt(2,t.getMatiere().getIdMatiere());
   ...
   public ObservableList<User> GetRole2(String role) {
      ...
      public ObservableList<String> GetNomMatiere() {
        ...
        return list;
             ...
             public ObservableList<Absence> OnlineAbsence(String source) {
        String req = "select u.*,a.*,m.* from Absence a inner join users u on a.id_user =u.idUser INNER JOIN matier m on m.id=a.id_matiere  where u.idUser = '"+source+"' ";
        ObservableList<Absence> list = FXCollections.observableArrayList();
        int n=0;
        try {
            ste = cnx.createStatement();
            ...
            //User user=new User(String.valueOf(id_user),nom_user,prenom_user,role_user);
            Matiere matiere=new Matiere(id_matiere,nom_matiere);
            Absence absence=new Absence(matiere,date_absence,timedeb_absence,timefin_absence);
            //// final etape bras omok e5dem 
            list.add(absence);
        ...
        return list;
                            ...
                            public Integer GetOnlineNbrAbsence(String id) {
      
        String req = "select count(*) from users u inner join classe c on u.classeEtd=c.Name  where u.idUser= '"+id+"' GROUP By u.idUser  ";
        ObservableList<Integer> list = FXCollections.observableArrayList();
        int n=0;
        try {
            ste = cnx.createStatement();

...
public class AbsenceService {
   ...
   public ObservableList<User> GetRole(String role) {
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new User (rs.getInt("id"),rs.getString("nom"),rs.getString("prenom"),rs.getString("role")));
            }
        } catch (SQLException ex) {
            Logger.getLogger(AbsenceService.class.getName()).log(Level.SEVERE, null, ex);
        }
      ...
      public ObservableList<String> GetNomMatiere() {
        ...
        String req = "select u.*,a.*,m.* from Absence a inner join user u on a.id_user =u.id INNER JOIN matiere m on m.id=a.id_matiere";
        ...
        try {
            ...
            int id_user = rs.getInt("u.id");
            String nom_user=rs.getString("u.nom");
            String prenom_user=rs.getString("u.prenom");
            String role_user=rs.getString("u.role");
            ...
            ///// importation
            User user=new User(id_user,nom_user,prenom_user,role_user);
        ...
        String req = "select u.*,a.*,m.* from Absence a inner join user u on a.id_user =u.id INNER JOIN matiere m on m.id=a.id_matiere  where u.nom like '%"+source+"%'";
        ObservableList<Absence> list = FXCollections.observableArrayList();
        int n=0;
        try {
            ste = cnx.createStatement();

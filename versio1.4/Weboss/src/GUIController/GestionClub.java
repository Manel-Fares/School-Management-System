...
//for second column we use value
    ...
    private void ajouter(MouseEvent event) {
        boolean isCinEmpty = ValidationChamps.isTextFieldNotEmpty(nom_clb, nClub,"Empty!");
        boolean isCinEmpty1 = ValidationChamps.isTextFieldNotEmpty(ID_resp, idResp,"Empty!");
        boolean isCinEmpty2 = ValidationChamps.isTextFieldNotEmpty(doma, Dom,"Empty!");
                final ObservableList<Club> listClub2 = FXCollections.observableArrayList();
       ...
       Club c=new Club(id_resp, nom, domainee);
        try {
            if(!isCinEmpty && !isCinEmpty1 &&!isCinEmpty2){
            cs.ajouter(c);
             listClub2.addAll(cs.affciher());
            tab_club.setItems(listClub2);}
        } catch (SQLException ex) {
